...
const Notes = ({userIdClicked, colorgroupChoice}) => {
    ...
    if (existinggroupNamesData) {
      ...
      return( existingNotes.map((note, index) => (
       (userIdClicked === note.id) ?(
        <div style={{ display: "flex" , flexDirection:"column", justifyContent: "space-between",margin: "10px", background:"white"}} key={index}>
          <div className={StyleNotes.notes} style={{}}> {note.notes}</div>
    ...
    <>
      {userIdClicked > 0 ? (
        <div className={StyleNotes.NotesGroupNotes}>
          ...
          <div className={StyleNotes.NotesEnter} style={{backgroundColor:`${colorgroupChoice}`}}>
           { console.log(colorgroupChoice, "colorgroupChoice")}
            <textarea
              type="text"
